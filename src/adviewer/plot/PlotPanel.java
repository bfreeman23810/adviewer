 ...
 * @author root
 */
public class PlotPanel extends JPanel implements ActionListener, ItemListener, ClipboardOwner, ImageListener, RoiListener, Runnable {
    ...
    public Thread bgThread;		// thread for plotting (in the background)
    ...
    public boolean showFit = false;
    public ADMainPanel adMain;
    
    public JLabel minFit;
    public JLabel maxFit;
    public JLabel centFit;
    public JLabel rmsFit;
     ...
     */
    public PlotPanel(PlotPlus plot, ImagePlus imp, boolean isX, boolean debug) {
        ...
        pc = new PlotCanvasPlus(this.imp, debug);
        pc.setPlot(plot);
    ...
    public void setLabels( JLabel min , JLabel max, JLabel cent, JLabel rms, JLabel twoRms , JLabel fwhm ){
        this.minFit = min;
        this.maxFit = max;
        this.centFit = cent;
        this.rmsFit = rms;
        this.twoRmsFit = twoRms;
        this.fwhmFit = fwhm;
    }
    
    
    public void writeToFitLabels(String min, String max, String cent, String rms , String twoRms, String fwhm){
        
        
        if(!isHorizontal){
            setLabels(adMain.minXVal , 
                    adMain.maxXVal ,
                    adMain.centroidXVal, 
                    adMain.rmsXVal,
                    adMain.twoRmsXVal,
        ...
        if(this.minFit !=null){
            ...
            this.minFit.setText(min);
            
        }
        if(this.minFit !=null){
            this.maxFit.setText(max);
        }
        if(this.minFit !=null){
            this.centFit.setText(cent);
        }
        if(this.minFit !=null){
            this.rmsFit.setText(rms);
        }
        if(this.twoRmsFit !=null){
            this.twoRmsFit.setText(twoRms);
        }
        if(this.fwhmFit !=null){
            this.fwhmFit.setText(fwhm);
        }
        
        //Log.log("returning from setLabels" , debug);
    }
    ...
    //set a reference to the main Panel
    public void setMainPanel(ADMainPanel main){
        this.adMain = main;
    }
    ...
    public void draw() {
        ...
        if (listValues) {
            ...
            pc.requestFocus();	//have focus on the canvas, not the button, so that pressing the space bar allows panning
        }        //plot.draw();
    ...
    public void stop() {
        ...
        //if(srcImp!=null){
        //  doUpdate = true;
        // notify(); // in case the thread is in wait modes
        //}
    }

    public void restartLive() {
        if (bgThread != null && bgThread.isAlive() && liveMode == true) {
            Log.log("Stopping thread", debug);
            stop();
            //bgThread=null;
        }
    ...
    void copyToClipboard(boolean writeAllColumns) {
        ...
        try {
            systemClipboard = getDefaultToolkit().getSystemClipboard();
        } catch (Exception e) {
     ...
     */
    public void drawPlot(PlotPlus plot) {
        this.plot = plot;
        if (imp != null) {
    ...
    public void enableLiveProfiling() {
        /* if (plotMaker == null) {
        ...
        liveMode = true;
        Log.log("live mode enabled", debug);
        if (srcImp == null) {
            return;
        }
    ...
    public void run() {
        Log.log("liveMode enabled ... ", debug);
        while (liveMode) {
            //  Log.log("running", debug);
            IJ.wait(SLEEPTIME);	//delay to make sure the roi has been updated
            ...
            try {
                ...
                Log.log("Error in getting plot", debug);
                if (debug) {
                    e.printStackTrace();
                }
            }
        }
        Log.log("exiting live mode  ", debug);
    }

    private void createListeners() {

       // Log.log("createListeners", debug);
